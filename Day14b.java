...
public class Day14b {
  ...
  public static void main(String[] args) {
    ...
    // add a floor that is 2 rows lower than lowest data input.
    for (int i = 0; i < 1000; ++i) {
      new Coord(i, yLimit + 2).addTo(occupied);
    }

    Coord newSand;
    Coord spawn = new Coord(500, 0);
    int count = 0;
    while ((newSand = Coord.restsAt(spawn, occupied, yLimit)) !=spawn) {
      ++count;
      newSand.addTo(occupied);
    }
    ++count; // for last sand that rests at spawn point.
